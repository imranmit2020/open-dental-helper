-- Retry migration with compatibility tweaks
...
SET search_path TO ''
AS $$
  SELECT COALESCE((SELECT role FROM public.profiles WHERE user_id = _user_id),'user');
$$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='profiles' AND policyname='Staff can view profiles'
  ) THEN
