...
interface DentistAvailabilityFormProps {
  dentistId?: string;
  availability?: any;
  onSuccess?: () => void;
  onCancel?: () => void;
...
export function DentistAvailabilityForm({ 
  dentistId, 
  availability, 
  onSuccess, 
  onCancel 
}: DentistAvailabilityFormProps) {
  const { createAvailability, updateAvailability } = useDentistAvailability();
  const { tenants, currentTenant } = useTenant();

  const form = useForm<AvailabilityFormData>({
    resolver: zodResolver(availabilitySchema),
    defaultValues: {
      dentist_id: dentistId || availability?.dentist_id || "",
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {/* You'll need to fetch dentists list */}
                    <SelectItem value="placeholder">Select a dentist</SelectItem>
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
            ...
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
              <div className="space-y-0.5">
                ...
                <div className="text-sm text-muted-foreground">
                  Toggle dentist availability for this schedule
                </div>
              </div>
              <FormControl>
                <Switch
